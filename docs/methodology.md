...
- [4. Mortality Component](#4-mortality-component)
  ...
  - [4.6 Operative Survival Path and Source Provenance](#46-operative-survival-path-and-source-provenance)
...
- [10. Limitations](#10-limitations)
  ...
  - [10.7 Open-Ended 90+ Survival and Race-Flat Mortality](#107-open-ended-90-survival-and-race-flat-mortality-adr-068)
  - [10.8 Additional Modeling Simplifications](#108-additional-modeling-simplifications-2026-vintage)
...
The resulting rate table is a complete $7 \times 7$ grid (seven age groups by seven race/ethnicity categories, including a "total" row). The downstream pipeline expands these five-year maternal-age groups to single-year-of-age rates by assigning each five-year ASFR uniformly to every single year within the band (`_expand_age_groups_to_single_years`); fertility is thus held **constant within each five-year maternal age band** (see §10.8). This flat expansion differs from the Sprague osculatory interpolation used for the base *population* age structure (ADR-048, §2.3): it preserves group-level births and TFR exactly but does not model within-band age curvature.

**Provenance note on the operative table.** The operative rate file (`data/raw/fertility/nd_asfr_processed.csv`) carries a single `year = 2023` column. This is a *pooling-window label* — the terminal year of the 2020--2023 pool — not a single-year (2023-only) estimate. Every value in that table is the four-year pooled ASFR defined above (pooled 2020--2023 births over summed 2020--2023 female population); the underlying CDC WONDER birth counts are themselves a 2020--2023 total (e.g., 9,859 White non-Hispanic births at ages 25--29 across the window, not a single year). See `scripts/data/build_nd_fertility_rates.py` (the `year` assignment is commented accordingly) and `data/raw/fertility/DATA_SOURCE_NOTES.md` ("2020-2023 combined ... pooled for stability").
...
The projection uses age 90 as the terminal open-ended age group. Because individuals in this group cannot "age out," its survival rate must describe retention across the *entire* open interval (ages 90 through the oldest survivors), not the single-year survival at exact age 90. The operative production rate is the open-interval survivorship ratio computed from the life table's $T_x$ column (total person-years lived above age $x$):

$$
S(90+, s) = \frac{T_{91}(s)}{T_{90}(s)}
$$

where $T_{91} = T_{90} - L_{90}$ is person-years lived above age 91. This is the demographically standard terminal-group survivorship ratio. Evaluated on the CDC 2023 life table (all-race "Total"), the base-year (2025) values are approximately **0.778 for males and 0.806 for females**; the Census NP2023 improvement trajectory (Section 4.5) then carries them gradually upward over the horizon (about 0.787 male / 0.816 female by 2045).

This open-interval rate (ADR-068) corrects an earlier build defect in which the grouped ND CDC 85+ rate (~0.885 male / ~0.914 female) was broadcast flat across ages 85--100, so the engine held the 90+ pool at a single-year-style rate far too high for an open group that contains the 95- and 100-year-olds. The correction is implemented as `apply_open_ended_survival_correction` in `data/process/mortality_improvement.py`: it overrides the operative `max_age` (90) survival with $T_{91}/T_{90}$ while preserving the NP2023 improvement *shape*. Ages above 90 are unused by the engine (population is capped at the open-ended group) and are left unchanged. See Section 10.7 for the magnitude (90+ population at 2055 reduced from ~13,707 uncorrected to 8,172) and the race-flat-mortality caveat. When the $T_x$ columns are unavailable, the static-base derivation (Section 4.1) falls back to $S(90+) = 1 - q_x(90)$; the operative production path always applies the $T_{91}/T_{90}$ correction above.
...
### 4.6 Operative Survival Path and Source Provenance

The mortality component draws on several life-table sources for different data years. These are **not** competing choices — they are distinct, sequential stages, each consuming the appropriate source. The production run uses the following three-stage path:

1. **Static race-specific base (Sections 4.1--4.2).** National race-specific life tables (NVSR 74-06, 2023 data) supply the race differentials; the ND/national $q_x$ ratio from the ND state life table (NVSR 74-12, 2022 data) and the matching 2022 national table calibrates the overall level to North Dakota. Output: `data/processed/survival_rates.parquet`. The configuration key `rates.mortality.life_table_year: 2023` is the base-year ($t_0$) anchor for the Lee-Carter improvement (Section 4.5) on **this** static table, matching the 2023 data year of the national race tables. This race-specific base is built and stored, but it is **superseded at runtime** by stage 2.

2. **Operative time-varying table (the table the engine actually consumes).** `data/process/mortality_improvement.py` builds `data/processed/mortality/nd_adjusted_survival_projections.parquet` as Census Bureau NP2023 national survival-ratio projections rescaled by an ND adjustment factor (ND CDC 2020 baseline ÷ Census national 2025), where the "ND CDC 2020 baseline" is the CDC ND 2020 life table distributed in the SDC 2024 dataset. This table is year-specific (2025--2055) and embeds national mortality improvement directly, so the engine sets its in-engine improvement factor to zero to avoid double-counting (Section 7.1, Step 1). **It carries no race dimension and is applied race-flat** across all six race groups — the disclosed limitation of Section 10.7 (ADR-068 D3).

3. **2023 open-age correction (Section 4.3; ADR-068).** `apply_open_ended_survival_correction` overrides the operative open-ended (age-90+) survival with the open-interval ratio $T_{91}/T_{90}$ taken from the CDC 2023 combined life table (`data/raw/mortality/cdc_lifetables_2023_combined.csv`, all-race "Total"), preserving the NP2023 improvement shape.

In short: **NVSR 74-06 (2023) + NVSR 74-12 (2022)** define the static race-specific base; **Census NP2023 × CDC ND 2020 baseline** is the operative race-flat table; and **CDC 2023 $T_x$** supplies only the age-90+ open-interval correction. The `life_table_year: 2023` config value pertains exclusively to stage 1.
...
Counties are identified using on-campus face-to-face enrollment as a percentage of county population, with a 2.5% inclusion threshold (ADR-061 Decision 4). Williams County (WSC, 1.5%) was originally included below threshold on the hypothesis that WSC enrollment produces measurable turnover artifacts, but was removed on 2026-06-11: clean-base benchmarking showed its inclusion *worsened* Williams MAPE by +0.90pp with no offsetting gains, confirming the double-dampening risk ADR-061 had flagged for a county that also receives oil-boom and male dampening (ADR-067). The production smoothing list is these 11 counties. Only on-campus students are counted; distance education enrollment is excluded since those students may reside anywhere.
...
| ADR-061 | College Fix Model Revision | Accepted (as modified, 2026-06-11): D1 + D2 (GQ f=0.75) + D4 (minus Williams) in production; D3 rejected for this release | 5 |
...
| ADR-067 | Ward/Grand Forks Divergence Investigation (Corrective) | Accepted (2026-06-13) | 5, 10.6 |

ADR-061 was dispositioned on 2026-06-11 (CF-001 Tier-3 decision) and is now **Accepted (as modified)**. Decision 1 (college-age smoothing and rate cap extended to ages 25-29) and Decision 4 (college county list expanded on enrollment evidence) are in `config/projection_config.yaml`, with Decision 4 modified by ADR-067 to **remove Williams** (11 college counties, not 12). Decision 2 (the GQ-fraction parameter) was adopted and calibrated to f=0.75. Decision 3 (extended convergence medium hold) was **rejected for the 2026 release** -- its under-projection justification was substantially a walk-forward harness artifact (ADR-067 F1) -- so `medium_hold_years` remains 10.
...
Group quarters population is held constant at 2025 levels through 2055 (Sections 2.5.4 and 7.3). Changes in institutional capacity -- base realignments, dormitory construction, nursing facility expansion -- are not modeled.

**Components of change are household-basis.** Because the engine projects only the household population and re-adds GQ as a constant at each year, the projected components of change -- births, deaths, and net migration -- describe the **household population only** and exclude GQ turnover (the births, deaths, and moves of people in dormitories, military barracks, and nursing facilities). Projected deaths are therefore on the order of ~5,000/year, below the ~7,000/year PEP total-population deaths; the gap is the unmodeled GQ component, **not** a mortality error. Any components-of-change figure presented in a public or internal output must carry this household-basis label. Separately, the ADR-055 Phase 2 correction, which subtracts historical GQ population from the snapshots used to compute migration rates (Section 5b), was calibrated on 2026-06-11: the EXP-C clean-base benchmark supported reducing the fraction from the $f = 1.0$ default to the production value $f = 0.75$ (best recent-origin state APE, modest rural and overall MAPE gains; forward effect ≈ +3.0k state @2050 relative to $f = 1.0$). The correction remains a material modeling choice: GQ-stable counties are unaffected by the fraction (the subtraction cancels out of period differences), while counties with GQ changes (e.g., Cass dormitory growth) carry the calibration. Confirmed at the 2026-06-13 locked-config production rerun: the GQ f=0.75 calibration contributes ≈ +3,600 state @2055 (≈ +3,000 @2050) of the locked baseline's position relative to f=1.0 (see `docs/plans/2026-public-projection-release-handoff/final-run-metadata.md`).
...
**Open-ended 90+ survival (corrected).** The operative survival build expanded the ND CDC 85+ group rate flat across ages 85-100, so the engine held the open-ended 90+ group at a single-year-style rate (~0.885 Male / 0.914 Female) every year -- too high for an open group that contains the 95- and 100-year-olds. This is corrected to the demographically proper open-interval survivorship ratio $T_{91}/T_{90}$ from the CDC 2023 life table (~0.778 Male / 0.806 Female at 2025, improving over the horizon), reducing the 90+ population at 2055 from ~13,707 (uncorrected) to **8,172** (full-horizon corrected). See `core/mortality.py` (open-ended group handling) and `apply_open_ended_survival_correction` in `data/process/mortality_improvement.py`.

**Operative survival must span the full horizon (amended 2026-06-16).** The open-interval correction and the NP2023 trajectory apply only where the operative survival table (`nd_adjusted_survival_projections.parquet`) provides year-specific rates. For any projection year absent from that table, the engine silently falls back to the static-base survival table (Section 4.6, stage 1) — bypassing both this correction and the NP2023 improvement. The 2026-06-15 run's operative table inadvertently spanned only 2025-2045, so for the steps 2047--2055 it reverted to the static base; correcting the table to the full 2025--2055 horizon moved 2055 from 886,585 to **898,907 (+12.45%)** and the 90+ pool from 9,971 to **8,172** (years 2025--2046 unchanged). A coverage guard in `scripts/pipeline/02_run_projections.py` now **hard-fails** production/public runs whose operative table does not span the full horizon (with an explicit `allow_static_survival` opt-out for tests and experiments), backed by a release-QA assertion; see ADR-068 (Amendment) and the PR #25 recurrence-hardening pass.

**Race-flat production mortality and migration (disclosed limitation, ADR-068 D3).** Although the survival method is *designed* to be race-specific (Section 4.2), the **operative** production survival -- the time-varying Census NP2023-adjusted table that takes precedence at runtime -- carries no race dimension and is applied identically across all six race groups; the race-specific static base (Section 4.2) is loaded but superseded at runtime. **Net migration is likewise race-flat:** the residual migration pipeline is computed at the county × age-group × sex level and each rate is expanded uniformly across all six races (Section 5; `expand_5yr_migration_to_engine_format`). Thus the only genuinely race-specific inputs to the production run are fertility (Section 3.2) and the base-year population composition; race-specific survival and migration are **not** modeled in 2026. The state-total effect of race-flat survival is modest (the dominant white-NH group ≈ the all-race total), but it over-states AIAN survival in the reservation counties (Benson, Sioux, Rolette), and race-flat migration similarly limits the fidelity of race/ethnicity-detailed outputs. For the 2026 vintage these are accepted, disclosed limitations; rebuilding the survival and migration pipelines to carry race is scheduled for the next vintage. Race/ethnicity-detailed projections should be read with these caveats.

### 10.8 Additional Modeling Simplifications (2026 Vintage)

The following are accepted simplifications for the 2026 release, disclosed here and scheduled for revisiting in a future vintage. None changes the state total materially; they are documented for transparency and reproducibility.

- **Small-county race-blend threshold not wired to config.** The processed county age-sex-race distribution file is prebuilt by `scripts/data/build_race_distribution_from_census.py`, which blends each county's race composition toward the statewide pattern with weight $\alpha = \min(\text{county pop}/5000, 1)$. The 5,000 threshold is a script default; the `base_population.county_distributions.blend_threshold` config key (also 5,000) is **not** read by that build step. The two values coincide for the 2026 run, but the config key should be wired through next vintage so the threshold is governed in one place.
- **Flat 5-year → single-year ASFR expansion.** The pipeline copies each five-year maternal-age ASFR flat to every single year of age within the band (`_transform_fertility_rates`); fertility is therefore **constant within each 5-year maternal age band**, not Sprague/graduated. This preserves group-level births and TFR but not within-band age curvature.
- **Newborns not exposed to infant mortality in their birth year.** Births enter as age 0 from the pre-migration population and are not subjected to a within-year infant survival factor (Section 7.1, Steps 2 & 4). Because infant survival is ~0.998--0.999, the effect is on the order of tens of persons per year statewide.
- **College-age smoothing updates the migration *rate*, not the `net_migration` diagnostic.** College-county smoothing (Section 5f) adjusts the operative `migration_rate` used by the engine, but residual-diagnostic columns labeled `net_migration` are not recomputed post-smoothing. The projection itself uses the smoothed rate; any published components should be derived from the projection outputs, not from stale post-smoothing `net_migration` diagnostic files.
- **CBO migration decrement is uniform across age/sex/race and applied after the rate cap.** The additive CBO reduction $\Delta r(t)$ (Section 6.1) subtracts the same per-capita decrement from every cell, after the age-aware convergence rate cap (Section 5i) has already been applied. The first-year corrected decrement (~0.00335) can therefore push an already-capped general rate slightly past the nominal cap. An immigrant-age-profile allocation is preferred for a future vintage.
