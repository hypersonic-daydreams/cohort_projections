...
Both files exclude city/place projections. Public geography levels are state, region, and county.
...
| Sheet | Purpose | Notes |
|-------|---------|-------|
| `README` | Plain-language file description, release date, contacts, methodology pointer, caveats | Include ADR-042 scenario wording. |
| `Scenario Summary` | Statewide key-year comparison across scenarios | Use 2025, 2030, 2035, 2040, 2045, 2050, 2055. |
| `State Annual` | Annual state totals and broad demographics | One row per scenario-year. |
| `Region Annual` | Annual region totals | One row per scenario-region-year. |
| `County Annual` | Annual county totals | One row per scenario-county-year. |
| `County Key Years` | Compact county table for public users | Key years only, with baseline-led columns and scenario range fields if approved. |
| `State Age Groups` | Broad state age-group composition | Include under 18, 18-64, 65+, and 85+. |
| `Data Dictionary` | Column definitions and scenario descriptions | Match CSV schema. |
...
- Tidy format.
- One row per scenario, geography, and year.
- Scenarios: `restricted_growth`, `baseline`, `high_growth`.
- Geographies: 1 state, 8 regions, 53 counties.
- Years: 2025-2055 inclusive.
- Expected row count after final production: `5,766`.

Required columns:

| Column | Type | Description |
|--------|------|-------------|
| `scenario` | string | Scenario key: `restricted_growth`, `baseline`, or `high_growth` |
