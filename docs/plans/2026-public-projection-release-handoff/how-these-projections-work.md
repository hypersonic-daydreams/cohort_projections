...
conjure a future from nothing. It is to take what we know about the present — carefully, in detail —
and follow it forward, year by year, watching how it changes as people age, as some are born, as
...
The cohort-component method works precisely because it refuses that shortcut. Rather than carry one
number per county, it carries a detailed portrait of the population — and it updates every feature
...
things can change the count over the course of a year. The method handles each in turn — and it is
from these three *components* that the method takes its name.
...
> mortality experience rather than the national average. They also improve gradually over time
...
has come not from births and deaths but from people on the move — a great deal of it from abroad.
...
> converge toward more moderate long-run levels rather than assumed to run at peak rates forever.
...
> age-zero cohort. State totals are then formed by **summing the 53 counties** rather than projecting
> the state on its own — so county, regional, and state figures always reconcile exactly.
...
**799,400**. That early dip is worth dwelling on for a moment, because it is easily misread: it is a
built-in feature of the migration assumption, not a downturn the model uncovered on its own.
...
> future. Change the assumptions — above all those about migration — and the path changes with them.

---

## What the statewide number conceals

If there is one point worth carrying away, it is this: the statewide total describes no single
county's experience. Under the baseline, growth is strikingly concentrated. Roughly three-quarters
of all projected gains fall in just three counties — **Cass** (Fargo), **Williams** (Williston), and
**Burleigh** (Bismarck) — while **37 of the 53 counties are projected to decline.**

Because the method follows each county on its own terms, reflecting its particular age structure and
its own history of migration, it can explain these divergences rather than merely assert them.
**Williams County** continues to grow largely because its population is unusually young and so
produces more births than deaths — this even after its in-migration is assumed to run at about half
its recent rate. **Ward County**, around Minot, declines because it has recorded net out-migration
...
own back-testing — launching the current model from past years and checking it against what actually
...
2. **No method anticipates a rupture.** An oil boom, a pandemic, a sharp turn in federal immigration
   policy — the model projects forward from its assumptions and cannot foresee the events that would
   overturn them. That is a limit of the enterprise, not a flaw in this particular model.
...
it honors the real structure of a population. Its limits are simply the limits of looking thirty
years ahead — the further out the gaze, and the smaller the place, the more the answer leans on
assumptions that the future is free to revise.

For exact annual figures by state, region, and county, see the public download package (the Excel
workbook and CSV). For the full technical methodology — every formula, data source, and special-case
