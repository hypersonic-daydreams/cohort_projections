...
- [x] **3.1 (G3a)** Components-of-change instrumentation **before** the rerun: persist annual state-level births/deaths/net-migration per run (engine change in `cohort_projections/core/cohort_component.py` or a post-hoc derivation script); tests pass. **Done 2026-06-12** — committed in `12fa6f9` ("feat(engine): persist annual components of change (PUB-2026 Stage 3.1)"); consumed by Stage 4.1 components reconciliation.
- [x] **3.2 (G4a)** Add "State Age-Sex Detail" sheet (5-year groups × sex, key years 2025–2055) to the public workbook export; update `public-download-spec.md` + data dictionary. **Done 2026-06-13** — `_build_state_age_sex_detail` in `build_public_draft_package.py` (36 rows, reconciles exactly to totals/sex); spec + README sheet guide updated.
- [x] **3.3 (B1 completion)** Production rerun under the locked config — **done 2026-06-13** (commit `12fa6f9`; `m2026r1`/`cfg-20260611-production-lock`; config sha256 `bf897444b5a4fec7`). Recorded in `final-run-metadata.md` (QA Gate 1).
- [x] **3.4 (R5b)** Alias/inactive scenarios — **retired**. The public release is baseline-only (ADR-065); `restricted_growth`/`high_growth` remain inactive internal sensitivities and are not regenerated at the locked vintage. The Stage 0.3 quarantine stands as the permanent disposition (stale-output READMEs in place; not part of any public artifact).
