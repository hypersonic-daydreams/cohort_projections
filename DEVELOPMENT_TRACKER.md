...
| Projection Observatory readiness | stable | Trust/correctness and operational hardening complete (`OBS-01` through `OBS-09`); bounded unattended queueing is now supported under sequential execution plus SOP-003 review governance. Deterministic autonomous search is now available through isolated bare-mirror/worktree execution with protected-path policy, recipe allowlists, patch capture, and canonical artifact harvesting. Search-only method cloning is now wired into the benchmark runtime so recipe candidates can benchmark sandbox-only method IDs without touching production aliases; the active recipe catalog now covers a broader search-only lattice across convergence-window widening, mortality-improvement sensitivity, and selected interaction terms. A one-command `search-auto` launcher now plans, batches, refreshes cache, and writes per-session candidate-summary CSV/JSON plus Markdown/HTML reports for more hands-off evidence gathering. The Projection Observatory dashboard Command Center now exposes autonomous-search session selection, preview controls, launch controls, stop controls, persisted progress bars, candidate/result previews, embedded search-report previews, and dashboard-captured launch-log tails for in-dashboard monitoring. A dedicated `History` tab now provides longitudinal benchmark-history views across all runs, champion-at-run baselines, metric delta heatmaps, category trend lines, and accepted/rejected challenger history. The follow-on roadmap is now implemented as well: cross-tab shortlist persistence, dense-table scanability cleanup, explicit promotion policy, benchmark-contract validation, promotion-package building, post-promotion revalidation, refined segmentation lenses, scope-aware bundle registration for future place benchmarks, and operational-quality tracking. |
...
| PP-008 | Deterministic autonomous search sandbox | completed_2026-03-15 | Added deterministic search planning/execution/reporting backed by a bare mirror plus disposable worktrees, protected-path policy, declarative recipe catalog, session state, patch/log harvesting, and CLI commands (`search-plan`, `search-run`, `search-status`, `search-report`) without any auto-promotion path. Follow-up on 2026-03-15 wired search-only method cloning into the benchmark runtime so enabled recipe candidates can benchmark sandbox-only method IDs and benchmark-only config knobs without mutating production aliases. A second 2026-03-15 follow-up added `search-auto` for one-command autonomous planning/batch execution/reporting plus candidate-summary session artifacts to support more hands-off configuration search and evidence review. A third 2026-03-15 follow-up broadened the enabled search-only recipe catalog into a small experimental lattice around convergence-window and mortality settings so unattended runs produce more decision-useful comparative statistics instead of isolated point tests. A fourth 2026-03-15 follow-up surfaced autonomous-search preview/launch/progress monitoring directly in the Projection Observatory dashboard Command Center via persisted search-session summaries and candidate previews. A fifth 2026-03-15 follow-up added dashboard-managed background-process sidecars so the Command Center can stop dashboard-launched searches and preview the search report, launch log, and best completed candidates without leaving the dashboard. | `scripts/analysis/observatory.py`, `cohort_projections/analysis/observatory/search_controller.py`, `cohort_projections/analysis/observatory/sandbox_manager.py`, `cohort_projections/analysis/observatory/recipe_registry.py`, `config/observatory_search_policy.yaml`, `config/observatory_recipes.yaml`, `docs/guides/observatory-autonomous-search.md`, `scripts/analysis/run_benchmark_suite.py`, `scripts/analysis/run_experiment.py`, `scripts/analysis/walk_forward_validation.py`, `cohort_projections/analysis/observatory/dashboard/data_manager.py`, `cohort_projections/analysis/observatory/dashboard/tab_command_center.py` |
