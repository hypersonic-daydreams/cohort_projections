...
| PP-008 | Deterministic autonomous search sandbox | completed_2026-03-15 | Added deterministic search planning/execution/reporting backed by a bare mirror plus disposable worktrees, protected-path policy, declarative recipe catalog, session state, patch/log harvesting, and CLI commands (`search-plan`, `search-run`, `search-status`, `search-report`) without any auto-promotion path | `scripts/analysis/observatory.py`, `cohort_projections/analysis/observatory/search_controller.py`, `cohort_projections/analysis/observatory/sandbox_manager.py`, `cohort_projections/analysis/observatory/recipe_registry.py`, `config/observatory_search_policy.yaml`, `config/observatory_recipes.yaml`, `docs/guides/observatory-autonomous-search.md` |
