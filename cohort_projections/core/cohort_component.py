...
from .fertility import calculate_births, apply_fertility_scenario, validate_fertility_rates
...
class CohortComponentProjection:
    ...
    def project_single_year(
        ...
        if not births.empty:
            ...
            combined_population = pd.concat(
                [population_with_migration, births],
                ignore_index=True
            )
        else:
    ...
    def run_projection(
        ...
        for year in range(start_year, end_year):
            try:
                ...
                current_population = self.project_single_year(
                    current_population,
                    year,
                    scenario
                )
    ...
    def _create_annual_summary(
        ...
        # Population under 18
        summary['population_under_18'] = population[
            population['age'] < 18
        ]['population'].sum()
        ...
        # Seniors (65+)
        summary['population_65_plus'] = population[
            population['age'] >= 65
        ]['population'].sum()
    ...
    def get_population_by_year(self, year: int) -> pd.DataFrame:
        ...
        year_data = self.projection_results[
            self.projection_results['year'] == year
        ].copy()
    ...
    def get_cohort_trajectory(
        ...
        cohort_data = self.projection_results[
            (self.projection_results['sex'] == sex) &
            (self.projection_results['race'] == race)
        ].copy()
    ...
    def export_results(
        ...
        if format == 'parquet':
            ...
            self.projection_results.to_excel(
                output_path,
                index=False,
                engine='openpyxl'
            )
