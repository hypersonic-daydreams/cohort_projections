...
def calculate_births(
    ...
    logger.debug(
        f"Year {year}: Calculating births for ages {reproductive_ages[0]}-{reproductive_ages[1]}"
    )
    ...
    births_by_mother = reproductive_pop.merge(
        fertility_rates, on=["age", "race"], how="left", suffixes=("_pop", "_rate")
    )
    ...
    births_by_mother["births"] = births_by_mother["population"] * births_by_mother["fertility_rate"]
...
def validate_fertility_rates(
    fertility_rates: pd.DataFrame, config: dict[str, Any] | None = None
) -> tuple[bool, list]:
    """
    ...
    if config:
        reproductive_ages = (
            config.get("rates", {}).get("fertility", {}).get("apply_to_ages", [15, 49])
        )
...
def apply_fertility_scenario(
    fertility_rates: pd.DataFrame, scenario: str, year: int, base_year: int
) -> pd.DataFrame:
    """
