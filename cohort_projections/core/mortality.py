...
def apply_mortality_improvement(
    ...
    logger.debug(
        f"Applied {years_elapsed} years of {improvement_factor:.4f} mortality improvement"
    )
...
def validate_survival_rates(
    ...
    for (sex, race), group in survival_rates.groupby(['sex', 'race']):
        ...
        if len(survival_values) > 10:
            # Check if early ages have lower survival than much older ages
            if np.mean(survival_values[:5]) < np.mean(survival_values[-5:]):
                issues.append(
