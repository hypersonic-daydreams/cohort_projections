...
def load_life_table_data(
    ...
    try:
        ...
        raise ValueError(f"Error reading life table file: {e}")
    ...
    if age_col is None:
        raise ValueError(
            "No age column found. Expected one of: age, age_group, agegrp"
        )
...
def calculate_survival_rates_from_life_table(
    df: pd.DataFrame,
    method: str = 'lx'
) -> pd.DataFrame:
    """
            ...
            if age < 90:
                ...
                if method == 'lx':
                    ...
                    if not next_age_data.empty:
                        ...
                        if 'qx' in group.columns:
                            survival_rate = 1 - group.iloc[idx]['qx']
                        else:
                            survival_rate = 0.95  # Default
                ...
                if 'tx' in group.columns and 'lx' in group.columns:
                    ...
                    if 'qx' in group.columns:
                        ...
                        survival_rate = 0.65  # Typical value
    ...
    logger.info(
        f"Calculated survival rates for {len(result_df)} age-sex-race combinations"
    )
...
def apply_mortality_improvement(
    ...
    base_year: int,
    projection_year: int,
    improvement_factor: float = 0.005
) -> pd.DataFrame:
    """
    ...
    avg_improvement = (
        improved_df['survival_rate'].mean() - df['survival_rate'].mean()
    )
    logger.info(
...
def create_survival_rate_table(
    ...
    if missing_count > 0:
        ...
        df_complete.loc[missing_mask, 'survival_rate'] = df_complete.loc[
            missing_mask, 'age'
        ].apply(get_default_survival_rate)
...
def validate_survival_rates(
    ...
    if (df['survival_rate'] > 1).any():
        ...
        validation_result['errors'].append(
            f"Survival rates > 1.0 found: {over_one_count} records"
        )
        validation_result['valid'] = False
    ...
    try:
        ...
        validation_result['warnings'].append(
            f"Could not calculate life expectancy: {e}"
        )
    ...
    if validation_result['valid']:
        ...
        logger.error(
            f"Survival rate validation failed with "
            f"{len(validation_result['errors'])} errors"
        )

    if validation_result['warnings']:
        logger.warning(
            f"Survival rate validation produced "
