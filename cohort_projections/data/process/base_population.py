...
def harmonize_race_categories(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"Harmonized {len(df)} records across "
                f"{df['race_ethnicity'].nunique()} categories")
...
def create_cohort_matrix(
    ...
    """
    logger.info(f"Creating cohort matrix for {geography_level} "
                f"{geography_id if geography_id else ''}")
    ...
    expected_races = [cat for cat in demographics['race_ethnicity']['categories']]
    ...
    cohort_matrix['processing_date'] = datetime.now().strftime('%Y-%m-%d')

    logger.info(f"Created cohort matrix with {len(cohort_matrix)} cells, "
...
def validate_cohort_matrix(
    df: pd.DataFrame,
    geography_level: str,
    expected_counties: Optional[int] = None
) -> Dict[str, any]:
    """
    ...
    expected_ages = list(range(
        demographics['age_groups']['min_age'],
        demographics['age_groups']['max_age'] + 1
    ))
    expected_sexes = demographics['sex']
...
def get_cohort_summary(cohort_matrix: pd.DataFrame) -> pd.DataFrame:
    ...
    summary_stats.append({
        'category': 'Total',
        'group': 'All',
        'population': cohort_matrix['population'].sum()
    })
    ...
    for group_name, min_age, max_age in age_groups:
        pop = cohort_matrix[
            (cohort_matrix['age'] >= min_age) &
            (cohort_matrix['age'] <= max_age)
        ]['population'].sum()
        summary_stats.append({
            'category': 'Age Group',
            'group': group_name,
    ...
    summary_df['percentage'] = (
        summary_df['population'] / cohort_matrix['population'].sum() * 100
    )
