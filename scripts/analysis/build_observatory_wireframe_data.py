...
.. note::
   The observatory wireframe was archived to ``docs/archive/observatory_wireframe/``
   on 2026-06-23 (repo-hygiene Decision 4). This generator is retained for history;
   its output path points into the archive.
"""
...
WIREFRAME_DIR = PROJECT_ROOT / "docs" / "archive" / "observatory_wireframe"
...
def per_county_metrics(cm: pd.DataFrame, champ_method: str, chall_method: str) -> pd.DataFrame:
    ...
    def agg(method: str) -> pd.DataFrame:
        ...
        g = (
            sub.groupby(["county_fips", "county_name", "category"])
            .agg(
                mape=("abs_pct", "mean"),
                bias=("pct_error", "mean"),
...
def build_scorecard(b: BenchmarkBundle, decision: dict) -> dict:
    ...
    metrics = [
        ...
        (
            "sentinel_grand_forks",
            "Sentinel - Grand Forks MAPE",
    ...
    if not path.exists():
        return {
            ...
            "verdict": _stub_verdict(
                "No decision record found at " + str(path.relative_to(PROJECT_ROOT))
            ),
        }
    txt = path.read_text()
...
def build_labels(b: BenchmarkBundle, decision: dict) -> dict:
    ...
    return {
        ...
        "decision": {
            ...
            "file": (
                f"docs/reviews/benchmark_decisions/{decision.get('decision_id', '')}.md"
...
def history_from_index() -> list[dict]:
    ...
    rows = idx.drop_duplicates("run_id").sort_values("run_date", ascending=False).head(10)
    out = []
...
def main() -> None:
    ...
    parser.add_argument(
        "--list-runs", action="store_true", help="print available benchmark runs and exit"
    )
