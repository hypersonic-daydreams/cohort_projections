...
\textbf{Small-sample inference robustness.} With only seven treated clusters (nationalities), conventional cluster-robust inference may be unreliable. Table~\ref{tab:bootstrap_inference} reports alternative $p$-values from wild cluster bootstrap and randomization inference. For the full pre-period specification, the wild cluster bootstrap yields $p = 0.084$ (not significant at $\alpha = 0.05$), while randomization inference yields $p = 0.043$ (significant at $\alpha = 0.05$). This sensitivity reflects the small number of treated clusters and suggests caution in claiming strong statistical significance.
...
\midrule
Full pre-period (2002--2017) & $-1.393$ & 0.031* & 0.084 & 0.043* \\
Restricted (2013--2017) & $-1.992$ & $<$0.001*** & 0.003** & 0.002** \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
