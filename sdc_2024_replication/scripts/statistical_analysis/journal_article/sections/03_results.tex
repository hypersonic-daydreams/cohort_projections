...
Rolling-origin backtesting (expanding window, 2010--2016 start; 2017--2024 forecasts) benchmarks point accuracy and interval calibration. Table~\ref{tab:backtesting} shows that the driver regression using contemporaneous U.S.\ migration achieves the lowest MAE/RMSE and relatively strong coverage, while the naive random-walk and ARIMA(0,1,0) baselines coincide. Interval widths remain large across models, and MAPE is inflated by the low 2020 base; these results are therefore descriptive rather than definitive.
...
\toprule
\textbf{Model} & \textbf{MAE} & \textbf{RMSE} & \textbf{MAPE} & \textbf{Cov. 80\%} & \textbf{Cov. 95\%} \\
\midrule
Naive RW & 1,153 & 1,367 & 313\% & 0.63 & 0.75 \\
Expanding mean & 1,674 & 1,980 & 635\% & 0.38 & 0.50 \\
Driver OLS (US) & 317 & 530 & 65\% & 0.88 & 0.88 \\
ARIMA(0,1,0) & 1,153 & 1,367 & 313\% & 0.63 & 0.75 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes}: Rolling-origin evaluation with expanding window; $n = 8$ one-step forecasts. Driver model uses contemporaneous U.S.\ migration (ex post). Coverage refers to empirical coverage of symmetric prediction intervals.
\end{tablenotes}
\end{table}
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Variable} & \textbf{Simple} & \textbf{Full Gravity} & \textbf{State FE} \\
\midrule
...
\begin{tablenotes}
\small
\item \textit{Notes}: Two-way clustered standard errors (state and origin) in parentheses. PPML estimation. * $p < 0.05$; *** $p < 0.001$. Dependent variable: LPR admissions by state-country pair, FY 2023. State FE specification includes 50 state dummy variables (coefficients not reported).
\end{tablenotes}
\end{table}
...
The full gravity specification adds origin and destination foreign-born totals. With two-way clustered SEs, the diaspora association attenuates to 0.140 (SE = 0.248, $p = 0.57$), while state foreign-born totals remain the dominant predictor (elasticity = 0.792, SE = 0.212, $p < 0.001$). The origin stock coefficient is small and imprecise (0.046, SE = 0.192).
...
The attenuation from 0.45 to approximately 0.14 has practical implications for forecasting. Much of the simple-model association reflects selection of large receiving states with both large diaspora stocks and continued large flows. The pseudo-$R^2$ values indicate that the full gravity model explains 40\% of deviance, roughly double the explanatory power of the bivariate specification.
...
The estimated average treatment effect on the treated (ATT) is $-1.384$ on the log scale ($t = -2.14$, $p = 0.032$), corresponding to a 74.9\% reduction in refugee arrivals from affected countries. The 95\% confidence interval [$-2.65$, $-0.12$] excludes zero, indicating a statistically significant and economically substantial short-run effect.

Parallel trends diagnostics are mixed. The linear pre-trend test yields a treated-country trend coefficient of 0.087 ($t = 0.97$, $p = 0.334$), but the event study's joint pre-treatment test rejects parallel trends over the full pre-period ($F = 4.31$, $p < 0.001$). Figure~\ref{fig:eventstudy} nevertheless shows the immediate post-treatment coefficients shifting sharply negative in 2018--2019, so we interpret the estimates as short-run policy effects and treat longer pre-period divergence with caution.
...
The level shift coefficient indicates that international migration dropped by 19,503 persons immediately following COVID onset ($t = -4.28$, $p < 0.001$). However, the trend change coefficient of $+$14,113 ($t = 4.29$, $p < 0.001$) indicates accelerated growth post-COVID, suggesting rapid recovery. Relative to pre-COVID mean migration of 15,669, the level shift represents a 124.5\% decline---interpretable as a transition from positive to negative net migration during the acute pandemic period.
...
\begin{table}[htbp]
\centering
\caption{Net International Migration (\PEP) Scenario Projections for North Dakota, 2045}
\label{tab:scenarios}
\begin{tabular}{@{}llc@{}}
\toprule
