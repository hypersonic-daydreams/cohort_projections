...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Break Year} & \textbf{Policy Event} & \textbf{$F$-statistic} & \textbf{$p$-value} & \textbf{Mean Shift} \\
\midrule
...
Because this panel block is an intercept-only benchmarking specification (state and year effects without additional covariates), fixed effects and random effects represent alternative parameterizations of state heterogeneity rather than competing causal specifications. The Hausman statistic is effectively zero ($H = 7.18 \times 10^{-31}$, $p = 1.000$), indicating that FE and RE estimates are numerically indistinguishable in this setting; we therefore do not interpret it as evidence favoring random effects. The Breusch-Pagan LM test strongly rejects pooled OLS in favor of a model with state-level heterogeneity ($\chi^2 = 1,502$, $p < 0.001$), confirming that between-state differences are substantial.
...
\begin{table}[htbp]
\centering
\caption{Policy-Associated Divergence and Predictive Relevance}
\label{tab:causal_effects}
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{Method} & \textbf{Policy} & \textbf{Estimate} & \textbf{SE} & \textbf{95\% CI} \\
\midrule
DiD (log scale) & Travel Ban & $-1.393$* & 0.647 & [$-2.66$, $-0.12$] \\
\quad \textit{Percentage effect} & & $-75.2\%$ & --- & --- \\
ITS Level Shift & COVID-19 & $-19,503$*** & 4,556 & [$-28,432$, $-10,573$] \\
ITS Trend Change & COVID-19 & $+14,113$*** & 3,288 & [$+7,667$, $+20,558$] \\
\midrule
Shift-share (first stage) & National shocks & 8.88*** & 1.16 & [$+6.61$, $+11.15$] \\
\quad First-stage $F$ & & 58.76 & & \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes}: DiD = Difference-in-differences with nationality and year fixed effects, SEs clustered by nationality. The reported ``percentage effect'' for DiD is computed as $100\times(\exp(\widehat{\text{ATT}})-1)$ from the $\ln(y+1)$ specification, so it is formally a percent change in $(y+1)$. ITS = Interrupted time series with state fixed effects, SEs clustered by state. Shift-share row reports first-stage relevance only (not a causal effect). * $p < 0.05$; *** $p < 0.001$. Travel Ban affected countries: Iran, Iraq, Libya, Somalia, Sudan, Syria, Yemen.
\end{tablenotes}
\end{table}

\subsubsection{Travel Ban Difference-in-Differences}

The Travel Ban analysis employs refugee arrival data spanning 2002--2019, with 2018 designated as the first full post-treatment year. The treatment group comprises seven affected nationalities (Iran, Iraq, Libya, Somalia, Sudan, Syria, Yemen), while 118 unaffected nationalities serve as controls. The sample includes 1,119 nationality-year observations.

The estimated average treatment effect on the treated (ATT) is $-1.393$ on the log scale ($t = -2.15$, conventional $p = 0.031$), corresponding to a policy-associated divergence of 75.2\% on the $(\text{arrivals}+1)$ scale implied by the $\ln(y+1)$ outcome. The 95\% confidence interval [$-2.66$, $-0.12$] excludes zero under conventional clustered-SE inference, though inference is sensitive with only seven treated nationalities (see robustness checks below).

Parallel trends diagnostics are mixed, requiring caution in causal interpretation. The linear pre-trend test yields a treated-country trend coefficient of 0.085 ($t = 0.94$, $p = 0.349$), but the event study's joint pre-treatment test rejects parallel trends over the full pre-period ($F = 4.22$, $p < 0.001$). Figure~\ref{fig:eventstudy} shows divergence beginning approximately 10 years before treatment, indicating that the treatment and control groups were on different trajectories prior to the policy. The immediate post-treatment coefficients shift sharply negative in 2018--2019, but because the parallel trends assumption is not supported in the full pre-period, the DiD estimate may conflate the policy impact with pre-existing divergence. We therefore interpret the 75\% estimate as policy-associated divergence (descriptive policy-sensitivity evidence) rather than a precise causal effect magnitude. Appendix Figure~\ref{fig:app_event_study_extended} reports a descriptive extension through FY2024 (ADR-027); post-2020 coefficients are not interpreted as standalone causal Travel Ban effects.

\textbf{Small-sample inference robustness.} With only seven treated clusters (nationalities), conventional cluster-robust inference may be unreliable. Table~\ref{tab:bootstrap_inference} reports alternative $p$-values from wild cluster bootstrap and randomization inference. For the full pre-period specification, the wild cluster bootstrap yields $p = 0.083$ (not significant at $\alpha = 0.05$). Randomization inference yields a two-sided $p = 0.057$ (not significant at $\alpha = 0.05$); a directionally motivated one-sided test yields $p = 0.041$. This sensitivity reflects the small number of treated clusters and suggests caution in claiming strong statistical significance.
...
\midrule
Full pre-period (2002--2017) & $-1.393$ & 0.031* & 0.083 & 0.057 \\
Restricted (2013--2017) & $-1.992$ & $<$0.001*** & 0.009** & 0.004** \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes}: WCB = Wild cluster bootstrap with Rademacher weights (9,999 iterations, two-sided test based on the treatment $t$-statistic). RI = Randomization inference with treatment permutation (9,999 permutations; two-sided $p$ shown). * $p < 0.05$; ** $p < 0.01$; *** $p < 0.001$.
\end{tablenotes}
\end{table}

\textbf{Restricted pre-period robustness.} Because the event study reveals greater pre-trend divergence in early years (2002--2012) than in years immediately preceding treatment, we re-estimate the DiD restricting to years where parallel trends appear more plausible. Restricting the pre-period to 2013--2017 substantially improves the pre-trend diagnostic: the joint $F$-test yields $F = 0.75$, $p = 0.562$, failing to reject parallel trends. The ATT estimate increases in magnitude to $-1.992$ (SE = 0.507), and inference strengthens: the conventional $p$-value falls to $p < 0.001$, with bootstrap ($p = 0.009$) and randomization ($p = 0.004$) inference confirming significance at the 1\% level.

This pattern---a more negative ATT magnitude in the restricted window alongside improved pre-trend diagnostics---is consistent with a sharp post-2017 treated--control divergence in refugee arrivals. However, because the restricted window is chosen based on diagnostics, it should be interpreted as a sensitivity check rather than a definitive identification strategy. Taken together, the full-period and restricted-period results indicate that the post-2017 divergence is large and robust in sign, while the causal magnitude remains uncertain due to violated parallel trends in the full pre-period, small treated-cluster count, and window-selection concerns.

\subsubsection{COVID-19 Interrupted Time Series (National System Diagnostic)}
...
The shift-share index combines baseline state shares with leave-one-out national origin shocks to summarize predicted refugee inflows. The first-stage $F$-statistic of 58.76 exceeds the conventional threshold of 10, supporting instrument relevance.

The estimated first-stage coefficient is 8.88 ($p < 0.001$), indicating that a one-unit increase in the shift-share index predicts an 8.88-unit increase in state-level international migration. Because no second-stage causal model is estimated, this result is interpreted as predictive relevance rather than a causal effect.
...
Duration analysis examines the lifecycle of immigration ``waves''---sustained periods of elevated arrivals from specific origin countries. Wave identification criteria require arrivals to exceed 150\% of baseline for at least two consecutive years. Using the extended FY2002--FY2024 refugee arrivals series, this procedure identifies 2,057 waves across 78 nationalities and 45 states.

Kaplan-Meier estimation yields a median wave duration of 3.0 years (mean = 3.48 years). Survival probabilities decline steadily over the wave lifecycle: 68.8\% of waves survive past year 2; 48.1\% survive past year 3; 33.8\% survive past year 4; and only 5.2\% survive past year 10. The censoring rate of 25.5\% reflects waves still active at the end of the observation period (FY 2024), including a large cohort of waves initiated in FY2021+.
...
\midrule
Q1 (Low) & 515 & 2.0 years & 2.59 years \\
Q2 & 514 & 3.0 years & 3.13 years \\
Q3 & 522 & 4.0 years & 3.61 years \\
Q4 (High) & 506 & 6.0 years & 4.62 years \\
\midrule
\multicolumn{4}{l}{\textit{Log-rank test}: $\chi^2 = 633.0$, $p < 10^{-136}$} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
The log-rank test decisively rejects equality of survival curves across intensity quartiles ($\chi^2 = 633.0$, $p < 10^{-136}$), confirming that higher-intensity waves persist substantially longer. The median survival differential between Q1 and Q4---two years versus six years---indicates that wave duration triples at the upper extreme of initial intensity.

Regional origin also influences wave persistence. European (mean duration = 5.93 years) and Asian (3.88 years) waves outlast those from Africa (3.58 years), the Middle East (3.44 years), the Americas (2.24 years), and the residual ``Other'' category (3.25 years). The log-rank test confirms significant regional heterogeneity ($\chi^2 = 114.2$, $p < 10^{-22}$).
...
\begin{itemize}
    \item \textbf{Log intensity}: HR = 0.591 ($p < 0.001$). Higher-intensity waves exhibit 41\% lower hazard of termination, confirming the protective effect of initial magnitude.
    \item \textbf{Early wave}: HR = 1.767 ($p < 0.001$). Waves beginning in the early observation period terminate faster, consistent with endpoint truncation and secular timing effects.
    \item \textbf{Peak arrivals}: HR = 0.565 ($p < 0.001$). Higher peak arrivals during the wave reduce termination hazard.
    \item \textbf{Americas origin}: HR = 1.772 ($p < 0.001$). Waves from the Americas terminate 77\% faster than the reference category (Africa).
    \item \textbf{Europe origin}: HR = 0.629 ($p < 0.001$). European-origin waves exhibit 37\% lower termination hazard than the reference category (Africa).
\end{itemize}

Figure~\ref{fig:cox} visualizes the Cox model hazard ratio estimates with 95\% confidence intervals.

The Cox model achieves a concordance index of 0.777, indicating good discriminative ability---the model correctly ranks wave pairs by survival time 78\% of the time. The proportional hazards assumption is satisfied based on the global Schoenfeld residual test.
...
\midrule
CBO (Jan 2026) & CBO U.S.\ net immigration scaled by ND \PEP share & 2,138 \\
Moderate & Dampened historical trend (50\% weight) & 7,048 \\
Pre-2020 Trend & Anchor 2019; continue 2010--2019 slope (+72/year) & 2,517 \\
Restrictive policy (0.65$\times$ Moderate) & Reduced admissions via SDC factor (0.65x) & 4,581 \\
Zero & No international migration & 0 \\
\midrule
Monte Carlo (Baseline) & Stochastic trend with $CV_{2045} = 0.39$ & 8,720 \\
\quad 50\% PI & & [6,431, 11,024] \\
\quad 95\% PI & & [3,011, 14,393] \\
Wave-adjusted Envelope & Baseline + hazard-based wave persistence draws & 9,079 \\
\quad 95\% Envelope & & [3,360, 14,774] \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes}: CBO = Congressional Budget Office national net immigration projection (January 2026) scaled by North Dakota's mean share of U.S.\ net international migration in the \PEP series. PI = baseline-only prediction interval from 25,000 Monte Carlo draws. The wave-adjusted envelope adds hazard-based wave persistence draws and should be interpreted as conservative because wave-driven variation may already be reflected in baseline forecast uncertainty. $CV_{2045}$ denotes the coefficient of variation of baseline-only simulated 2045 outcomes (historical CV is 82.5\%). Baseline 2024 value = 5,126.
\end{tablenotes}
\end{table}

The five deterministic scenarios yield substantially different endpoints. The CBO (Jan 2026) scenario follows the national net immigration path projected by the Congressional Budget Office and scales it to North Dakota using the state's mean share of U.S.\ net international migration in the \PEP series, yielding 2,138 by 2045. The Moderate scenario, applying 50\% dampening to historical trends, yields 7,048. The Pre-2020 Trend scenario anchors at the 2019 level and extrapolates the 2010--2019 slope, projecting 2,517---reflecting the modest pre-pandemic growth rate. The restrictive-policy scenario applies a 35\% reduction to the Moderate trajectory (0.65$\times$), yielding 4,581 by 2045. The Zero scenario provides a floor of 0.

		Monte Carlo simulation propagates baseline parameter uncertainty through the projection model. Based on 25,000 draws from estimated trend and volatility distributions ($CV_{2045} = 0.39$), the baseline-only median 2045 projection is 8,720 persons, with a 50\% prediction interval of [6,431, 11,024] and a 95\% prediction interval of [3,011, 14,393]. Adding hazard-based wave persistence draws yields a conservative wave-adjusted uncertainty envelope: the median 2045 projection rises to 9,079 with a 95\% envelope of [3,360, 14,774]. The wave-adjusted envelope spans a factor of 4.4, reflecting substantial irreducible uncertainty over a 20-year horizon given the observed volatility and structural instability. The simulation CV refers to dispersion in projected 2045 outcomes and is not directly comparable to the historical CV of annual observations.

		Diagnostics from multivariate and cross-sectional models are reported for triangulation and benchmarking. Scenario paths are defined by explicit, policy-indexed rules rather than by weighting forecasts across specifications.
...
The fan chart visualization (Figure~\ref{fig:scenarios}) depicts the scenario trajectories with nested Monte Carlo uncertainty bands: a baseline-only prediction interval and an outer wave-adjusted uncertainty envelope that adds hazard-based persistence draws. For readability, the Zero counterfactual (a flat line at $y=0$) is omitted from the figure while remaining part of the reported scenario set (Table~\ref{tab:scenarios}). The nested presentation makes uncertainty sources transparent and avoids implicitly treating stacked stochastic components as a single calibrated prediction interval.
