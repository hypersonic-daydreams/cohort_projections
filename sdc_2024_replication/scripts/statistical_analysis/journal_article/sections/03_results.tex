...
The estimated average treatment effect on the treated (ATT) is $-1.393$ on the log scale ($t = -2.15$, conventional $p = 0.031$), corresponding to a policy-associated divergence of 75.2\% in refugee arrivals from affected countries. The 95\% confidence interval [$-2.66$, $-0.12$] excludes zero under conventional clustered-SE inference, though inference is sensitive with only seven treated nationalities (see robustness checks below).
