...
The analysis draws on four primary data sources---Census \PEP, RPC refugee arrivals, DHS \LPR{} admissions, and the \ACS---each capturing distinct dimensions of international migration to North Dakota. We define a single forecasting estimand based on the Census Bureau Population Estimates Program (\PEP) net international migration component for North Dakota, and treat all non-\PEP series (including multiple series derived from \ACS and RPC archives) as auxiliary inputs for decomposition, prediction, and validation. Table~\ref{tab:data_sources} maps each series to its measurement properties (flow versus stock; net versus gross; time basis) and its role relative to the forecast target.
...
\textbf{Time base and harmonization.} \PEP estimates are reported on an estimate-year basis, while DHS and RPC series are reported in fiscal years and \ACS estimates in survey years. We preserve native time bases and label them explicitly. When fiscal-year series are used as predictors for \PEP-year outcomes, we use an explicit month-overlap crosswalk when monthly data are available (RPC refugees and related humanitarian arrivals) and an overlap-weighted approximation \(X^{\text{PEP}}_{t} \approx 0.75\,X^{\text{FY}}_{t} + 0.25\,X^{\text{FY}}_{t-1}\) for annual-only FY sources (e.g., DHS \LPR), reporting one-year-shift sensitivity where timing is ambiguous. Implementation decisions and robustness choices are documented in \emph{Analysis Decision Records} (ADRs) included in the replication materials; ADR references in the text (e.g., ADR-025) point to those notes.
...
RPC refugee arrivals & Refugee (and related humanitarian) arrivals by nationality and state of initial placement & Flow & Gross & Fiscal year/month (FY; 2002--2024) & Mechanism and scenario input for the refugee component; supports policy responsiveness and wave modeling. Initial placement $\neq$ final residence (secondary migration); FY/month timing differs from \PEP but month-level data enable exact FY$\rightarrow$\PEP-year alignment; post-2020 state panels are partially missing (ADR-025). \\
\DHS\ \LPR{} admissions & Lawful permanent resident admissions by origin (state of intended residence) & Flow & Gross & Fiscal year (FY; 2000--2023 totals; 2023 origin detail) & Predictor and composition diagnostics (origin structure; gravity-style features). Intended residence may differ from actual; excludes other statuses; not net of outflows; multi-year panels can reflect reporting lags and documentation changes. \\
...
\ACS{} moved-from-abroad & International movers (lived abroad 1 year ago; MOE) by state and citizenship group (B07007) & Flow (proxy) & Gross (proxy) & Survey year (1-year est.; 2010--2023) & Proxy for recent inflows used for validation and sensitivity. Survey-based with MOE and small-cell sparsity; overlaps conceptually with \PEP allocation inputs, so treat as a correlated signal rather than an independent measure. \\
\bottomrule
\end{tabularx}
\endgroup
\end{table}
...
The Department of Homeland Security (DHS) publishes annual statistics on lawful permanent resident (LPR) admissions by state of intended residence and country of birth \citep{DHSYearbook2023}. This analysis uses FY2023 origin-by-state tabulations for cross-sectional composition and allocation models and a multi-year FY2000--FY2023 panel of state totals for time-series context and forecasting covariates. The LPR data provide granular information on the country-of-origin composition of legal immigration flows and the scale of durable admissions over time.

The principal advantage of DHS data lies in their administrative provenance: unlike survey-based measures, LPR counts derive from admission records and represent realized legal permanent residence. Limitations include exclusion of non-LPR categories (refugees, asylees, temporary workers), the distinction between state of intended residence at admission and actual destination after secondary migration, and potential reporting lags or documentation changes in yearbook tabulations. We therefore use stable aggregates for longitudinal context and interpret small year-to-year changes cautiously.

\subsubsection{American Community Survey Foreign-Born Population}

The American Community Survey (ACS) provides annual estimates of foreign-born population by place of birth for states and metropolitan areas \citep{CensusACS2023}. This analysis employs five-year estimates from 2009--2023, using the detailed country-of-birth tabulations (Table B05006) \citep{ACS_B05006_2023}. The foreign-born stock data enable analysis of diaspora effects on new migration, origin-specific growth trajectories, and Location Quotient calculations comparing North Dakota's immigrant composition to national patterns.
...
To proxy recent international inflows, we also extract ACS Table B07007 (Geographical Mobility in the Past Year by Citizenship Status) and use the ``moved from abroad'' counts by state and citizenship group (2010--2023) \citep{ACS_B07007_2023}. These series are not administrative flows: they are survey-based, carry margins of error, and conceptually overlap with inputs used in \PEP international migration allocation. We therefore treat moved-from-abroad estimates as a correlated signal used for validation and sensitivity rather than an independent measure of international migration.

\subsubsection{Refugee Processing Center Arrival Data}

The Refugee Processing Center (RPC), operated by the Department of State, publishes U.S.\ Refugee Admissions Program arrivals by state of initial resettlement and nationality \citep{RPCAdmissionsArrivals2026, RPCArchives2026}. We construct a unified FY2002--FY2024 series by combining an ORR/PRM academic dataset for FY2002--FY2011 \citep{DreherLanglotzMatzatParsons2020} with WRAPS/RPC tabulations for FY2012--FY2020 and RPC archive PDFs for FY2021--FY2024, enabling duration analysis of migration ``waves'' from specific origin countries. For recent years, RPC archives provide month-level reports, which we use to construct an explicit FY-month$\rightarrow$\PEP-year crosswalk for exact time-base alignment. RPC archives also provide Amerasian and Special Immigrant Visa (SIV) arrivals; we extract these as a parallel humanitarian series rather than forcing an early merge into refugee totals, and treat SIV as a separate durable channel in forecasting with capacity linkage and a conservative post-2024 sunset (ADR-026).

Refugee arrival data offer precise counts (not estimates) of a well-defined population admitted through federal resettlement programs. Limitations include restriction to initial placement, which may differ from eventual settlement after secondary migration, and incomplete state panels in some post-2020 RPC archive reports (states omitted in the source are left missing rather than zero-filled; ADR-025). Analyses that require a balanced state$\times$nationality panel (e.g., wave duration across states) therefore restrict the post-2020 window to states with complete FY2021--FY2024 archive coverage (ADR-025) and complete each state$\times$nationality annual series by inserting zero-arrival years before applying the wave rule, allowing duration analysis to use the full FY2002--FY2024 window without spurious adjacency when sources omit zero years.
...
\begin{enumerate}
    ...
    \item \textbf{Extended robustness window (2000--2024, $n=25$)}: Sensitivity analyses incorporate the Vintage 2009 data (2000--2009) to assess whether conclusions remain stable under the longer series. These results are reported as robustness checks rather than primary findings, and we explicitly model potential vintage effects through vintage-aware specifications.
\end{enumerate}

Throughout the analysis, we refer to the concatenated 2000--2024 data as a ``spliced \PEP-vintage series'' to signal that it bridges distinct measurement vintages rather than representing a methodologically homogeneous time series. Appendix Figure~\ref{fig:app_pep_regime_diagnostic} provides a measurement-vintage diagnostic visualization and reports vintage-specific trend slopes under a piecewise specification with an explicit COVID-19 intervention.

\subsubsection{Vintage-Aware Modeling and Sensitivity Analysis}

To assess whether vintage transitions introduce spurious patterns, we employ piecewise trend models that allow slope and intercept shifts at vintage boundaries (2010 and 2020). The estimated piecewise slopes reveal substantial differences across vintage periods: the 2000--2009 period exhibits a modest positive trend of 39 persons per year ($\text{SE} = 17.8$), the 2010--2019 period shows an insignificant trend of 72 persons per year ($\text{SE} = 72.3$, $p = 0.32$), and the 2020--2024 period displays a steep positive trend of 1,401 persons per year ($\text{SE} = 84.7$). A formal test of slope equality across vintage periods strongly rejects the null hypothesis ($F_{2,19} = 124.5$, $p < 0.001$), confirming that the extended series exhibits distinct temporal dynamics across vintage boundaries.
...
\end{equation}
and tests $H_0: \gamma = 0$ (unit root) against $H_1: \gamma < 0$ (stationarity). Lag length $p$ is selected by the Akaike Information Criterion (AIC). The Phillips-Perron (PP) test \citep{PhillipsPerron1988} provides a nonparametric alternative that corrects for serial correlation without augmenting the regression. Reported ADF and PP results use a constant-only specification (no deterministic trend), consistent with Appendix Table~\ref{tab:app_unitroot}. KPSS tests are run with a constant-only specification (level-stationarity). Break-robust Zivot-Andrews tests (single endogenous break) are reported as sensitivity checks. Where tests yield conflicting conclusions, the analysis reports both and interprets results cautiously.
...
Structural break analysis employs three complementary approaches. The Chow test \citep{Chow1960} evaluates whether regression parameters differ across known candidate break points by comparing residual sums of squares from restricted (single-segment) and unrestricted (segment-specific) models. The CUSUM test \citep{BrownDurbinEvans1975} detects parameter instability through cumulative sums of recursive residuals; departures from the zero line indicate structural change. For endogenous break detection, the analysis employs the \citet{BaiPerron1998} procedure as a constant-only mean-shift change-point detector (breaks in the mean level of $y_t$), identifying multiple break points by minimizing the sum of squared residuals across all possible segmentations:
\begin{equation}
\label{eq:bai_perron}
...
\end{equation}
where $\mathbf{y}_t = (y_t^{\text{ND}}, y_t^{\text{US}})^\top$ is a $2 \times 1$ vector containing North Dakota and national international migration, $\mathbf{A}_i$ are coefficient matrices, and $\boldsymbol{\varepsilon}_t$ is a vector white noise process (bold symbols denote vectors/matrices). Lag order selection employs AIC, and Granger causality tests evaluate directional predictive relationships. Forecast error variance decomposition quantifies the contribution of each variable to forecast uncertainty at various horizons.
...
Panel benchmarking exploits variation across states and time to characterize international migration levels in the national state system. The analysis employs a balanced state-year panel spanning 51 states (including the District of Columbia) over 15 years (2010--2024), yielding 765 observations.

We estimate an intercept-only two-way effects model:
\begin{equation}
\label{eq:fixed_effects}
y_{it} = \mu + \alpha_i + \lambda_t + \varepsilon_{it}
\end{equation}
where $y_{it}$ denotes international migration to state $i$ in year $t$, $\mu$ is the grand mean, $\alpha_i$ captures time-invariant differences across states (benchmarking relative scale), and $\lambda_t$ captures common year shocks (national system dynamics). No additional time-varying covariates are included in this panel block; it is therefore interpreted as a descriptive two-way effects decomposition rather than a causal estimate of covariate effects.

For completeness, we also report pooled OLS and state random-effects variants of the same intercept-only specification. Because the specification contains no substantive time-varying regressors, the fixed-versus-random effects distinction is not treated as a model-selection exercise; the Hausman statistic \citep{Hausman1978} is reported only as a diagnostic that the two parameterizations yield essentially identical estimates in this setting.
...
Cross-sectional allocation models quantify how LPR admissions are distributed across state--origin pairs as a function of diaspora stocks and destination characteristics. Although inspired by the gravity framework \citep{Tinbergen1962, AndersonVanWincoop2003}, our specification differs in two respects: (1) it uses a single FY2023 cross-section rather than bilateral panel data, and (2) it omits origin--destination distance. We omit physical distance because our destination units are U.S.\ states receiving flows from the same origin countries; variation in distance to a given origin is minimal and dominated by within-U.S.\ allocation patterns (e.g., origin--state distance variability is negligible compared to international distances). We therefore focus on diaspora networks and destination scale as the primary allocation mechanisms.\footnote{Traditional gravity models estimate distance effects using cross-country bilateral flows where distance proxies transaction costs.}
...
\begin{equation}
\label{eq:gravity}
E[M_{od} \mid X_{od}] = \exp(\beta_0 + \beta_1 \ln(\text{Stock}_{od} + 1) + \beta_2 \ln(\text{OriginTotal}_o + 1) + \beta_3 \ln(\text{DestTotal}_d + 1) + \boldsymbol{\gamma}'\mathbf{Z}_{od})
\end{equation}
where $M_{od}$ denotes LPR admissions from origin country $o$ to destination state $d$ (FY2023), $X_{od}$ denotes the right-hand-side covariate set, $\text{Stock}_{od}$ is the existing diaspora (foreign-born from $o$ residing in $d$), $\text{OriginTotal}_o$ is the national foreign-born population from origin $o$, $\text{DestTotal}_d$ is the total foreign-born population in destination state $d$, and $\mathbf{Z}_{od}$ contains additional bilateral controls. Log covariates are computed as $\ln(x+1)$ (log1p) to retain zero-stock pairs and avoid undefined $\ln(0)$. The coefficient $\beta_1$ on diaspora stock captures a cross-sectional diaspora--flow association useful for prediction.
...
\end{equation}
where $\lambda$ controls overall regularization strength and $\alpha \in [0,1]$ balances sparsity-inducing $L_1$ penalty against coefficient-shrinking $L_2$ penalty. The objective is written up to a constant scaling of the squared-error term, which can be absorbed into $\lambda$; the implementation follows scikit-learn's conventional $(1/2n)$ loss scaling. Cross-validation selects $(\lambda, \alpha)$ values. Coefficients shrunk exactly to zero identify variables excluded from the predictive model.
...
\begin{equation}
\label{eq:kmeans}
\arg\min_{\mathcal{C}, \{\boldsymbol{\mu}_k\}_{k=1}^{K}} \sum_{k=1}^{K} \sum_{i \in C_k} \|\mathbf{x}_i - \boldsymbol{\mu}_k\|^2
\end{equation}
...
Causal inference methods are used to assess whether major policy interventions---the 2017 Travel Ban and the 2020 COVID-19 pandemic---are associated with detectable divergences in migration flows. The primary design is a nationality-level difference-in-differences; under parallel trends it estimates an average treatment effect on the treated, but when diagnostics indicate pre-treatment divergence we interpret results as policy-associated divergence rather than a precise causal effect magnitude. A state-level interrupted time series provides a national system diagnostic for the COVID-19 structural break. Two additional components provide context: a synthetic comparator for descriptive benchmarking and a shift-share index used to assess the predictive relevance of refugee-driven shocks.
...
\end{equation}
where $y_{ct}$ denotes arrivals from country $c$ in year $t$, $\alpha_c$ and $\lambda_t$ are country and year fixed effects, and the coefficient $\delta$ estimates the average treatment effect on the treated (ATT) under the parallel trends assumption. The logarithmic transformation accommodates the multiplicative nature of policy effects; the addition of unity handles zero arrivals.
Because the outcome is $\ln(y_{ct}+1)$, $\exp(\delta)-1$ is formally the proportional effect on $(y_{ct}+1)$; when arrivals are not near zero, this closely approximates the percent change in arrivals.
...
\textbf{Restricted pre-period robustness.} When the joint pre-trend test rejects parallel trends over the full pre-period, we re-estimate the DiD restricting to years where pre-trends appear more plausibly parallel. This restriction is treated as a sensitivity analysis rather than a fully pre-specified identification strategy: it can improve diagnostics but may introduce window-selection concerns. If event study coefficients show greater divergence in early years than in years immediately preceding treatment, restricting the pre-period focuses attention on the years closest to treatment at the cost of reduced sample size.

\subsubsection{Interrupted Time Series (National System Diagnostic)}

To characterize the global migration shift associated with the pandemic, we employ a state-level interrupted time series (ITS) model estimated on the full 50-state panel. This analysis is intended as a \emph{national system diagnostic} to quantify the average state-level shock, not as a North Dakota-specific impact estimate. The model specifies:
\begin{equation}
\label{eq:its}
...
\end{equation}
where $\omega_{od,t_0}$ is state $d$'s share of national arrivals from origin $o$ in the baseline year $t_0$ (here $t_0=2010$) and $g_{o,t}^{\text{US},-d}$ is the corresponding leave-one-out \emph{level change} in national arrivals from origin $o$ between $t_0$ and $t$ (excluding state $d$ in both years). Thus $g$ is a change in persons (not a growth rate). Because no second-stage causal model is estimated, the results are interpreted as first-stage relevance: the strength of the relationship between predicted refugee inflows and state-level international migration. Standard errors are clustered by state; first-stage F-statistics assess instrument strength, with values exceeding 10 indicating adequate relevance.
...
To avoid baseline-window drift when extending the observation endpoint, baselines are computed as the median FY2002--FY2016 arrivals for each state$\times$nationality (a fixed pre--Travel Ban window). To ensure that ``two consecutive years'' denotes consecutive fiscal years (not consecutive non-missing observations), missing years in each state$\times$nationality series are treated as zero arrivals prior to wave identification. Waves still active in FY2024 are treated as right-censored.
...
\small
	...
	2.1 (ARIMA) & Baseline drift ($\hat{\beta}$); residual variance ($\hat{\sigma}^2$) & ARIMA point forecasts anchor Moderate; variance feeds MC simulation & Moderate \\
	2.2 (VAR) & FEVD proportions; Granger causality; ND--US comovement diagnostics & Diagnostic evidence on multivariate dynamics; does not parameterize scenario trajectories & None (diagnostic) \\
	3--4 (Panel/Robust) & State/year effects; between/within variance ratio & Validates ND position in state distribution; informs plausibility bounds & All (validation) \\
...
\end{table}
	...
	As Table~\ref{tab:module_outputs} indicates, the VAR model (Module 2.2) and machine learning methods (Module 6) serve primarily as auxiliary analyses. The VAR provides diagnostic evidence on ND--US comovement (e.g., forecast error variance decomposition (FEVD) and Granger causality tests) but is not used to generate scenario trajectories because long-horizon VAR forecasting would require future national migration values unavailable at forecast time. Similarly, ML methods (Elastic Net, Random Forest, K-means clustering) provide feature importance rankings and state clustering for comparative context, but do not directly parameterize the scenario trajectories. These modules support triangulation and validation rather than primary forecast generation.

\subsection{Scenario Construction}
\label{subsec:scenario_methods}

Scenario analysis translates analytical findings into forward-looking projections while explicitly characterizing uncertainty. The analysis develops five policy-indexed scenarios spanning the range of plausible future trajectories:

\begin{enumerate}
	    \item \textbf{CBO (Jan 2026)}: Uses the Congressional Budget Office's January 2026 national net immigration projection and scales it to North Dakota using the state's mean share of U.S.\ net international migration in the \PEP series (2010--2024). This scenario provides an externally benchmarked national baseline rather than an assumed growth rule \citep{CBODemographicOutlook2026}.
	    ...
	    \item \textbf{Restrictive policy (0.65$\times$ Moderate)}: Modifies the Moderate baseline by applying a 0.65 multiplier to future flows ($65\%$ of baseline). The multiplier is a scenario design choice informed by the Travel Ban DiD policy-associated divergence in refugee arrivals and a share-based scaling that translates a humanitarian-channel shock to the \PEP\ net international migration total.

	    \begin{center}
	    \fbox{\begin{minipage}{0.95\linewidth}
	    \small
	    \textbf{Calibration note (policy multiplier).} Let $\Delta_{\mathrm{hum}}$ denote a proportional shock to the refugee/humanitarian channel and let $s_{\mathrm{hum}}$ denote its share of the total migration flow. If non-humanitarian streams are unchanged, the implied multiplier on the \PEP\ total is:
	    \[
	        m_{\mathrm{policy}} = 1 - s_{\mathrm{hum}} \cdot \Delta_{\mathrm{hum}}.
	    \]
		    We parameterize $\Delta_{\mathrm{hum}} \in [0,1]$ as a reduction magnitude (e.g., $\Delta_{\mathrm{hum}} = 0.75$ denotes a 75\% decrease in the humanitarian channel). Module~7 estimates a Travel Ban DiD policy-associated divergence of approximately $-75.2\%$ in refugee arrivals from affected origins, so we take $\Delta_{\mathrm{hum}} \approx 0.75$ as a reference magnitude for calibration. To motivate $s_{\mathrm{hum}}$ using existing series (no new data pulls), we compare ND refugee arrivals (RPC) and ND LPR admissions (DHS) over 2010--2016: refugees comprise 0.383--0.492 of measured durable inflows (Refugees + LPR). This implies $m_{\mathrm{policy}} \in [0.630, 0.712]$; the chosen 0.65 multiplier corresponds to $s_{\mathrm{hum}} \approx 0.466$. Because 0.65 lies within this implied interval, we retain it in the main specification. This calibration is presented for transparency and should not be interpreted as an identified causal effect on the \PEP\ total.
	    \end{minipage}}
	    \end{center}
...
\end{enumerate}
	...
	Monte Carlo simulation propagates uncertainty through projection models by sampling from estimated parameter distributions. The procedure draws 25,000 realizations of model parameters, generates projected trajectories for each draw, and summarizes the resulting distribution through percentiles. The 50\% prediction interval spans the 25th to 75th percentiles; the 95\% prediction interval spans the 5th to 95th percentiles. To ensure reported 2045 summaries are not an artifact of a particular random seed, we ran a 10-seed stability check (seeds 42--51; 25,000 draws each) and found negligible Monte Carlo variation in the reported medians and 95\% endpoints (replication materials); we fix seed 42 for reproducibility of the main artifact. To avoid overstating precision when combining multiple stochastic components, we report two nested uncertainty summaries: (i) a baseline-only prediction interval from the univariate projection process and (ii) an outer wave-adjusted uncertainty envelope that adds hazard-based wave persistence draws (Module 8). The wave-adjusted envelope is intentionally conservative and should not be interpreted as a strictly calibrated predictive probability, because wave-driven variation may already contribute to baseline forecast uncertainty.

	The scenario engine does not combine forecasts via information-criterion weights. Instead, scenarios are defined by explicit, policy-indexed rules applied to the univariate baseline and trend components described above.
