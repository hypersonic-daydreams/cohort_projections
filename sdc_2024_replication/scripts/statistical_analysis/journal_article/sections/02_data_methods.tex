...
Throughout the analysis, we refer to the concatenated 2000--2024 data as a ``spliced \PEP-vintage series'' to signal that it bridges distinct measurement regimes rather than representing a methodologically homogeneous time series. Appendix Figure~\ref{fig:app_pep_regime_diagnostic} provides a measurement-regime diagnostic visualization and reports regime-specific trend slopes under a piecewise specification with an explicit COVID-19 intervention.
...
\begin{enumerate}
	    ...
	    \item \textbf{Immigration Policy}: Modifies the Moderate baseline by applying a 0.65 multiplier to future flows ($65\%$ of baseline). The multiplier is a scenario design choice informed by the Travel Ban DiD policy-associated divergence in refugee arrivals and a share-based scaling that translates a humanitarian-channel shock to the \PEP\ net international migration total.
