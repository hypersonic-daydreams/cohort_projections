...
Refugee arrival data offer precise counts (not estimates) of a well-defined population admitted through federal resettlement programs. Limitations include restriction to initial placement, which may differ from eventual settlement after secondary migration, and incomplete state panels in some post-2020 RPC archive reports (states omitted in the source are left missing rather than zero-filled; ADR-025). Analyses that require a balanced state$\times$nationality panel (e.g., wave duration across states) therefore restrict to FY2002--FY2020, while national-total and North Dakota-focused descriptive series use the extended FY2021--FY2024 window where available.
