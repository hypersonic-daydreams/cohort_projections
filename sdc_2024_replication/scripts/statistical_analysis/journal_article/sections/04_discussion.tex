...
The Location Quotient findings provide the clearest evidence of this distinctive profile. Egypt (LQ = 15.13), Sudan (LQ = 8.21), and various African regions exhibit extraordinary overrepresentation relative to national immigrant composition. These patterns are not accidents of economic migration but rather artifacts of humanitarian policy: federal refugee resettlement programs, operating through local voluntary agencies, have systematically directed arrivals from conflict zones to North Dakota communities \citep{Bansak2018, RefugeeMidwest2022}. The resulting ethnic enclaves---now spanning generations in some cases---create the foundation for network effects that shape subsequent migration streams.
...
The finding that North Dakota international migration follows an I(1) process---a random walk without drift---carries profound implications for forecasting. The optimal ARIMA(0,1,0) specification indicates that the best forecast of future migration equals the most recent observation, with innovations representing unpredictable shocks. This characterization implies that migration levels exhibit no mean reversion: departures from historical averages do not self-correct but instead persist indefinitely.

For practitioners, the random walk finding counsels humility. The five-year-ahead prediction interval spanning from 212 to over 10,000 migrants reflects not merely statistical imprecision but fundamental unpredictability in the migration process. Standard demographic projection methods that assume stable parameters or mean-reverting dynamics may generate misleadingly precise forecasts when applied to migration time series with these properties \citep{HyndmanAthanasopoulos2021, Cerqueira2019}.
...
\subsubsection{Causal Effects of Policy Interventions}

The causal inference findings provide the study's most policy-relevant contributions. The difference-in-differences estimate indicates that refugee arrivals from Travel Ban-affected countries declined by approximately 75\% following policy implementation---a substantial and statistically significant effect ($p = 0.004$) that satisfies the parallel trends assumption. This magnitude exceeds what aggregate analysis could detect because the Travel Ban's effects concentrated on specific nationalities while flows from other origins continued or even accelerated.

The 75\% reduction estimate aligns with administrative accounts of the Travel Ban's impact but provides rigorous causal attribution that prior descriptive analyses lacked. The finding that parallel trends held prior to 2017 ($p = 0.183$) strengthens confidence that the estimated effect reflects policy impact rather than pre-existing divergence between treated and control nationalities. For future policy evaluation, this analysis demonstrates that nationality-level panel methods can identify effects that aggregate time series approaches miss.

The COVID-19 analysis reveals a different causal mechanism: an immediate level shift of approximately 19,500 migrants (nationwide), followed by accelerated post-pandemic recovery. The recovery coefficient of +14,113 migrants per year suggests that immigration systems can ``bounce back'' from acute shocks more rapidly than the random walk characterization might imply. This finding offers some optimism for demographic planners concerned about the lasting effects of pandemic disruption.

\subsubsection{Forecast Uncertainty and Scenario Range}

The scenario projections yield a 2045 range spanning 2,517 to 19,318 migrants---nearly an order of magnitude---depending on policy assumptions. The Monte Carlo 95\% credible interval of 3,183 to 14,104 honestly characterizes the parametric uncertainty even within a given scenario. This wide range reflects multiple uncertainty sources: inherent migration volatility (CV = 82.5\%), structural instability evidenced by the 2020 break, and the fundamental unpredictability of future immigration policy.
...
The gravity model findings invite comparison with the established migration literature. The diaspora elasticity of 0.096---obtained from the full gravity specification controlling for origin and destination mass---falls substantially below estimates reported in prior cross-national studies. \citet{BeineDocquierOzden2011} estimate diaspora elasticities ranging from 0.4 to 0.7 in their analysis of bilateral migration determinants, while \citet{Mayda2010} reports network coefficients of similar magnitude in panel specifications.

Several factors may explain North Dakota's attenuated network effect. First, the state's migration composition tilts heavily toward refugees, whose placement decisions are mediated by resettlement agencies rather than purely by network connections \citep{Bansak2018}. Second, North Dakota's small absolute foreign-born population may generate insufficient network density to support the information transmission and chain migration mechanisms that produce larger diaspora effects in traditional gateway states. Third, the cross-sectional PPML specification, while methodologically appropriate following \citet{SantosSilvaTenreyro2006}, captures static associations that may understate the dynamic effects of network accumulation over time.

The finding that destination population dominates migration allocation (elasticity = 0.755) accords with gravity model theory and prior empirical work. \citet{AndersonVanWincoop2003} emphasize that bilateral flows reflect multilateral resistance---the attractiveness of alternatives---rather than purely bilateral factors. In this framework, North Dakota's modest population and peripheral geographic position generate substantial resistance that network effects cannot fully overcome.
...
Third, the difference-in-differences identification strategy assumes that absent the Travel Ban, refugee arrivals from affected and unaffected countries would have followed parallel trajectories. While the pre-treatment trend test supports this assumption ($p = 0.183$), alternative explanations---such as differential conflict dynamics across origin countries coinciding with policy implementation---cannot be definitively excluded.

Fourth, the synthetic control method for constructing counterfactual North Dakota trajectories relies on donor states (Wyoming, Vermont, Rhode Island) that may imperfectly represent what North Dakota would have experienced absent policy intervention. The excellent pre-treatment fit (RMSPE = 0.020) provides reassurance, but small-state donors may share unobserved characteristics that bias the counterfactual.
