...
The causal inference findings provide the study's most policy-relevant contributions. The difference-in-differences estimate indicates that refugee arrivals from Travel Ban-affected countries declined by approximately 75\% following policy implementation---a substantial and statistically significant effect ($p = 0.032$) in the first full post years. This magnitude exceeds what aggregate analysis could detect because the Travel Ban's effects concentrated on specific nationalities while flows from other origins continued or even accelerated.

Parallel trends diagnostics are mixed. The linear pre-trend test does not reject parallel trends ($p = 0.334$), but the event study joint pre-treatment test rejects when the full pre-period is included ($p < 0.001$), suggesting longer-run divergence across origin groups. We therefore interpret the estimated effect as a short-run policy response centered on 2018--2019 and treat longer-horizon dynamics with caution. For future policy evaluation, this analysis demonstrates that nationality-level panel methods can identify compositional effects that aggregate time series approaches miss.

The COVID-19 analysis reveals a different causal mechanism: an immediate level shift of approximately 19,500 migrants (nationwide), followed by accelerated post-pandemic recovery. The recovery coefficient of +14,113 migrants per year suggests that immigration systems can ``bounce back'' from acute shocks more rapidly than the random-walk baseline might imply. This finding offers some optimism for demographic planners concerned about the lasting effects of pandemic disruption.
